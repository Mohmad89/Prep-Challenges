...
const filterLinks = (str) => {
    ...
    for (let i = 0; i < arr.length; i++){
        if(arr[i].indexOf('//') !== -1){
            first = arr[i].indexOf('//') + 2;
            if(arr[i].indexOf('com') !== -1){
                last = arr[i].indexOf('com')+2;
            }if (arr[i].indexOf("org") !== -1) {
                last = arr[i].indexOf("org") + 2;
            }if (arr[i].indexOf("net") !== -1) {
                last = arr[i].indexOf("net") + 2;
            }
        result = arr[i].substring(first, last+1);
        }
    }
    return result;
...
const isPalindrome = (str) => {
    ...
    let arr = a.split(" ");
    let news_Arr = arr.join("").toLowerCase().split("");
    let string1 = arr.join("").toLowerCase();
    let string2 = "";
    for (let i = news_Arr.length - 1; i >= 0; i--) {
        string2 += news_Arr[i];
    }
